...
class ScalperSettingsIn(BaseModel):
    ...
    min_hold_bars: Optional[int] = None          # girişten sonra sinyal-zayıf çıkışı cooldown (5m bar)
...
_SCALPER_SETTING_KEYS = {
    ...
    "min_hold_bars": "scalper_min_hold_bars",
}
...
def api_set_scalper_settings(body: ScalperSettingsIn, db: Session = Depends(get_db)):
    ...
    updates = {
        ...
        "min_hold_bars": body.min_hold_bars,
    }
...
def _reset_portfolio_internal(db: Session, portfolio_slug: Optional[str] = None) -> dict:
    ...
    # stop() event set eder ve thread'in bitmesini bekler (join, timeout 10s).
    try:
