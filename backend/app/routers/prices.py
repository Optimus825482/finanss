...
def _fetch_prices_sync(portfolio_slug: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            portfolio = {
                "cash": p.get("cash", 0), "position_count": p.get("position_count", 0),
                "total_cost": p.get("total_cost", 0), "total_market_value": p.get("total_market_value", 0),
                "total_pl": p.get("total_pl", 0), "total_pl_pct": p.get("total_pl_pct", 0),
                "positions": [{
                    "id": pos.get("id"), "ticker": pos.get("ticker"), "quantity": pos.get("quantity"),
                    "entry_price": pos.get("entry_price"), "current_price": pos.get("current_price"),
                    "change_pct": pos.get("change_pct"), "unrealized_pl": pos.get("unrealized_pl", 0),
...
async def stream_prices(
    ...
    portfolio_slug = portfolio_slug.lower()
    if portfolio_slug not in VALID_SLUGS:
        raise HTTPException(status_code=400, detail=f"Geçersiz portföy: {portfolio_slug}")
    ...
    async def event_generator():
        while True:
            if await request.is_disconnected():
                break
            try:
                data = await asyncio.to_thread(_fetch_prices_sync, portfolio_slug)
